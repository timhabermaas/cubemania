...
function showTooltip(x,y,content){$("#tooltip").html(content);$("#tooltip").css({top:y+5,left:x+5});$("#tooltip").fadeIn(100);}
...
$("#times #chart").live("plothover",function(event,pos,item){if(item){showTooltip(item.pageX,item.pageY,item.series.tooltips[item.dataIndex]);}else{hideTooltip();}});$(document).ready(function(){if($("#times #chart").length==0)
