...
class User < ActiveRecord::Base
  ...
  has_many :singles, :order => 'created_at desc', :dependent => :delete_all do
    ...
    def best(puzzle_id); find_by_puzzle_id puzzle_id, :order => 'time'; end
  end
  ...
  validates_length_of :wca, :maximum => 10
  validates_format_of :wca, :with => /[0-9]{4}[A-Z]{4}[0-9]{2}/, :message => 'is not a valid WCA ID'
